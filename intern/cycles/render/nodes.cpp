...
NishitaSpectralSkyTextureNode::NishitaSpectralSkyTextureNode() : TextureNode(get_node_type())
{
}
...
void ConvertNode::constant_fold(const ConstantFolder &folder)
{
  ...
  if (to == SocketType::SPECTRAL && !folder.scene->integrator->get_spectral_rendering()) {
    to = SocketType::COLOR;
  }
...
NODE_DEFINE(PrincipledBsdfNode)
{
  ...
  SOCKET_IN_SPECTRAL(base_color, "Base Color", make_float3(0.8f));
  SOCKET_IN_SPECTRAL(subsurface_color, "Subsurface Color", make_float3(0.8f));
  SOCKET_IN_FLOAT(metallic, "Metallic", 0.0f);
  SOCKET_IN_FLOAT(subsurface, "Subsurface", 0.0f);
  SOCKET_IN_SPECTRAL(subsurface_radius, "Subsurface Radius", make_float3(0.1f));
...
NODE_DEFINE(SubsurfaceScatteringNode)
{
  ...
  SOCKET_IN_SPECTRAL(radius, "Radius", make_float3(0.1f));
...
NODE_DEFINE(EmissionNode)
{
  ...
  SOCKET_IN_SPECTRAL(color, "Color", make_float3(0.8f));
...
NODE_DEFINE(PrincipledVolumeNode)
{
  ...
  SOCKET_IN_SPECTRAL(color, "Color", make_float3(0.5f));
  SOCKET_IN_FLOAT(density, "Density", 1.0f);
  SOCKET_IN_FLOAT(anisotropy, "Anisotropy", 0.0f);
...
NODE_DEFINE(PrincipledHairBsdfNode)
{
  ...
  SOCKET_IN_SPECTRAL(tint, "Tint", make_float3(1.0f));
...
NODE_DEFINE(LightPathNode)
{
  ...
  SOCKET_OUT_SPECTRAL(ray_wavelength, "Ray Wavelength");
...
void LightPathNode::compile(SVMCompiler &compiler)
{
  ...
  out = output("Ray Wavelength");
  if (!out->links.empty()) {
    compiler.add_node(NODE_LIGHT_PATH, NODE_LP_ray_wavelength, compiler.stack_assign(out));
  }
...
GaussianSpectrumNode::GaussianSpectrumNode() : ShaderNode(get_node_type())
{
}
...
BlackbodySpectralNode::BlackbodySpectralNode() : ShaderNode(get_node_type())
{
}
...
MapRangeSpectrumNode::MapRangeSpectrumNode() : ShaderNode(get_node_type())
{
}
...
NODE_DEFINE(SpectrumMathNode)
{
  ...
  static NodeEnum spectrum_math_type_enum;
  spectrum_math_type_enum.insert("add", NODE_SPECTRUM_MATH_ADD);
  spectrum_math_type_enum.insert("subtract", NODE_SPECTRUM_MATH_SUBTRACT);
  spectrum_math_type_enum.insert("multiply", NODE_SPECTRUM_MATH_MULTIPLY);
  spectrum_math_type_enum.insert("divide", NODE_SPECTRUM_MATH_DIVIDE);
  spectrum_math_type_enum.insert("power", NODE_SPECTRUM_MATH_POWER);
  spectrum_math_type_enum.insert("logarithm", NODE_SPECTRUM_MATH_LOGARITHM);
  spectrum_math_type_enum.insert("minimum", NODE_SPECTRUM_MATH_MINIMUM);
  spectrum_math_type_enum.insert("maximum", NODE_SPECTRUM_MATH_MAXIMUM);
  spectrum_math_type_enum.insert("absolute", NODE_SPECTRUM_MATH_ABSOLUTE);
  ...
  SOCKET_IN_SPECTRAL(spectrum1, "Spectrum1", make_float3(0.5f));
  SOCKET_IN_SPECTRAL(spectrum2, "Spectrum2", make_float3(0.5f));

  SOCKET_OUT_SPECTRAL(spectrum, "Spectrum");

  return type;
}

SpectrumMathNode::SpectrumMathNode() : ShaderNode(get_node_type())
{
}
...
SpectrumCurvesNode::SpectrumCurvesNode() : ShaderNode(get_node_type())
{
}
