                            ...
                            BL::ShaderNode &b_node)
{
  ...
  if (b_node.is_a(&RNA_ShaderNodeSpectrumCurve)) {
    ...
    BL::CurveMapping mapping(b_curve_node.mapping());
    BL::CurveMap map_spectrum = mapping.curves[0];
    SpectrumCurvesNode *curves = graph->create_node<SpectrumCurvesNode>();

    mapping.update();

    array<float> curve_mapping_curves;
    curve_mapping_curves.resize(RAMP_TABLE_SIZE);
    for (int i = 0; i < RAMP_TABLE_SIZE; i++) {
      float x = mix(MIN_WAVELENGTH, MAX_WAVELENGTH, (float)i / (RAMP_TABLE_SIZE - 1));
      curve_mapping_curves[i] = mapping.evaluate(map_spectrum, x);
    }

    curves->set_curve(curve_mapping_curves);
    node = curves;
  }
  if (b_node.is_a(&RNA_ShaderNodeVectorCurve)) {
    ...
    math_node->set_spectrum_math_type((NodeSpectrumMathType)b_spectrum_math_node.operation());
    math_node->set_use_clamp(b_spectrum_math_node.use_clamp());
    node = math_node;
  }
    ...
    gaussian_spectrum->set_normalize(b_gaussian_spectrum.normalize());
