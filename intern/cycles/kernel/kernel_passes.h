    ...
    INTEGRATOR_STATE_ARGS, const ShaderData *sd, ccl_global float *ccl_restrict render_buffer)
{
  ...
  if ((sum_weight == 0.0f) || (sum_nonspecular_weight * 4.0f > sum_weight)) {
    ...
    if (kernel_data.film.pass_denoising_albedo != PASS_UNUSED) {
      ...
      const SpectralColor denoising_albedo = ensure_finite(denoising_feature_throughput *
                                                           diffuse_albedo);
                                                ...
                                                ccl_global float *ccl_restrict render_buffer)
{
  ...
  if (flag & PASSMASK(DIFFUSE_COLOR)) {
    const SpectralColor throughput = INTEGRATOR_STATE(path, throughput);
    kernel_write_pass_spectral_color(INTEGRATOR_STATE_PASS,
  ...
  if (flag & PASSMASK(MIST)) {
    ...
    const SpectralColor throughput = INTEGRATOR_STATE(path, throughput);
    const SpectralColor alpha = shader_bsdf_alpha(kg, sd);
