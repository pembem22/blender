...
ccl_device_inline bool shadow_volume_shader_sample(INTEGRATOR_STATE_ARGS,
                                                   ...
                                                   SpectralColor *ccl_restrict extinction)
{
...
ccl_device void volume_shadow_heterogeneous(INTEGRATOR_STATE_ARGS,
                                            Ray *ccl_restrict ray,
                                            ShaderData *ccl_restrict sd,
                                            SpectralColor *ccl_restrict throughput,
                                            const float object_step_size)
{
...
ccl_device SpectralColor volume_distance_pdf(float max_t, SpectralColor sigma_t, float sample_t)
{
  SpectralColor full_transmittance = volume_color_transmittance(sigma_t, max_t);
...
ccl_device SpectralColor volume_emission_integrate(VolumeShaderCoefficients *coeff,
                                                   int closure_flag,
                                                   SpectralColor transmittance,
                                                   float t)
{
...
volume_integrate_heterogeneous(INTEGRATOR_STATE_ARGS,
                               Ray *ccl_restrict ray,
                               ShaderData *ccl_restrict sd,
                               ccl_addr_space SpectralColor *ccl_restrict throughput,
                               const RNGState *rng_state,
  ...
  for (int i = 0; i < max_steps; i++) {
    ...
    if (volume_shader_sample(INTEGRATOR_STATE_PASS, sd, &coeff)) {
      ...
      if ((closure_flag & SD_SCATTER) || (has_scatter && (closure_flag & SD_EXTINCTION))) {
        ...
        const SpectralColor albedo = safe_divide(coeff.sigma_s, coeff.sigma_t);
        ...
        if (1.0f - xi >= sample_transmittance) {
          ...
          const SpectralColor new_transmittance = volume_color_transmittance(coeff.sigma_t,
                                                                             new_dt);
          const SpectralColor pdf = coeff.sigma_t * new_transmittance;
      ...
      if (closure_flag & SD_EMISSION) {
        const SpectralColor emission = volume_emission_integrate(
