...
ccl_device_inline SpectralColor stack_load_spectral(float *stack, uint a)
{
  ...
  return load_spectral_color(stack + a);
}
...
ccl_device_inline void stack_store_spectral(float *stack, uint a, const SpectralColor &spectral)
{
  ...
  store_spectral_color(spectral, stack + a);
}
  ...
  while (1) {
    ...
    switch (node.x) {
      ...
      case NODE_CLOSURE_BSDF:
        svm_node_closure_bsdf(kg, state, sd, stack, node, type, path_flag, &offset);
        break;
...
#  endif   /* NODES_FEATURE(NODE_FEATURE_BUMP) */
      ...
      case NODE_RGB_TO_SPECTRUM:
        ...
        break;
      case NODE_FLAT_SPECTRUM:
        svm_node_flat_spectrum(kg, state, stack, node.y, node.z);
        break;
...
#  endif /* NODES_FEATURE(NODE_FEATURE_HAIR) */
      ...
      case NODE_SPECTRUM_CURVES:
        svm_node_spectrum_curves(kg, state, stack, node, &offset);
        break;
      case NODE_TANGENT:
