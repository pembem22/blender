...
#  endif
      ...
      if (bsdf) {
        ...
        switch (parametrization) {
          case NODE_PRINCIPLED_HAIR_DIRECT_ABSORPTION: {
            SpectralColor absorption_coefficient = stack_load_spectral(stack,
                                                                       absorption_coefficient_ofs);
          ...
          case NODE_PRINCIPLED_HAIR_PIGMENT_CONCENTRATION: {
            ...
            SpectralColor melanin_sigma = bsdf_principled_hair_sigma_from_concentration(
                kg, eumelanin, pheomelanin, state->wavelengths);
          ...
          case NODE_PRINCIPLED_HAIR_REFLECTANCE: {
            SpectralColor color = stack_load_spectral(stack, color_ofs);
          ...
          default: {
            ...
            bsdf->sigma = bsdf_principled_hair_sigma_from_concentration(
                kg, 0.0f, 0.8054375f, state->wavelengths);
            break;
          }
        }
    ...
    case CLOSURE_BSSRDF_RANDOM_WALK_ID: {
      ...
      if (bssrdf) {
        ...
        bssrdf->radius = stack_load_spectral(stack, data_node.z) * param1;
                                           ...
                                           int *offset)
{
  ...
  if (emission > CLOSURE_WEIGHT_CUTOFF) {
    SpectralColor emission_color = stack_load_spectral(stack, emission_color_offset);
  ...
  if (blackbody > CLOSURE_WEIGHT_CUTOFF) {
    ...
    if (intensity > CLOSURE_WEIGHT_CUTOFF) {
      //   SpectralColor blackbody_tint = stack_load_spectral(stack, node.w);
      //   SpectralColor bb = blackbody_tint * intensity * svm_math_blackbody_color(T);
...
{
  SpectralColor weight = stack_load_spectral(stack, weight_offset);
                                         ...
                                         uint4 node)
{
  ...
  SpectralColor weight = stack_load_spectral(stack, color_offset) * strength;
