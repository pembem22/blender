...
static void ui_draw_but_curve_grid2(const uint pos,
                                    const rcti *rect,
                                    const float zoom_x,
                                    const float zoom_y,
                                    ...
                                    const float step_y)
{
  const float start_x = (ceilf(offset_x / step_x) * step_x - offset_x) * zoom_x + rect->xmin;
  const float start_y = (ceilf(offset_y / step_y) * step_y - offset_y) * zoom_y + rect->ymin;

  const int line_count_x = ceilf((rect->xmax - start_x) / (step_x * zoom_x));
  const int line_count_y = ceilf((rect->ymax - start_y) / (step_y * zoom_y));
